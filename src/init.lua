...
local function apply_phone_state(device, idx, seen_online, found_in_payload, now)
  ...
  local offline_key = "offline_count_" .. idx

  if not mac then
    device:set_field("missing_since_" .. idx, nil, { persist = true })
    device:set_field(offline_key, 0, { persist = true })
    emit_component(device, component, false)
    return false, false
  end

  -- Presence is fast: one positive SRM result immediately restores PRESENT.
  if seen_online then
    device:set_field("missing_since_" .. idx, nil, { persist = true })
    device:set_field(offline_key, 0, { persist = true })
  ...
  -- Away is conservative: require consecutive negative SRM polls.  Phones can
  -- briefly appear offline while idle/power-saving even though Wi-Fi remains on.
  local count = tonumber(device:get_field(offline_key) or 0) or 0
  count = count + 1
  device:set_field(offline_key, count, { persist = true })

  local required = tonumber(pref(device, "offlineConfirmations", 3)) or 3
  if required < 2 then required = 2 end
  if required > 10 then required = 10 end

  if count >= required then
    emit_component(device, component, false)
    log.info(string.format("PHONE%d confirmed AWAY after %d consecutive offline polls (%s)", idx, count, found_in_payload and "offline" or "not-found"))
    return false, true
  end

  -- Fail-safe for door automation: a transient negative result never creates
  -- an away event. Keep PRESENT until the confirmation threshold is reached.
  emit_component(device, component, true)
  log.info(string.format("PHONE%d offline candidate %d/%d (%s) - keeping PRESENT", idx, count, required, found_in_payload and "offline" or "not-found"))
  return true, true
end
